...
async function getMergeRequestsIds(projectFullPath, to, from, apiKey) {
...
async function getIssues(projectFullPath, to, from) {
    const getIssues =
        `query {
         project(fullPath: "${projectFullPath}") {
...
async function getMergeRequestInfo(projectFullPath, mrId, apiKey) {
    const getMergeInfo =
        `query {
          project(fullPath: "${projectFullPath}") {
          fullPath
...
async function generateReportIssues(infoWorksheet, tableData) {
...
async function generateReport(tableData, page, fileName, tableData1, infoWorksheet) {
...
function addWorksheet(worksheet, id) {
...
async function main(timea, timeb, worksheet, fileName, infoWorksheet, apiKey) {
    let tableData = []
    let tableData1 = []
    let flag = false
...
async function firstPage(apiKey, timea, timeb, infoWorksheet) {
    let tableData1 = []
    const allProjects = await getProjects(apiKey);
